...
import {
  ...
  publishRequirementsSchema,
  type TournamentDraftValues,
...
// createTournamentDraft / updateTournament 共通のフォームデータ抽出処理。
// フィールドを1つ追加・削除する際は、ここ1箇所を直せば両方に反映される。
function extractRawTournamentInput(formData: FormData) {
  return {
...
type ParseResult =
  | { success: true; data: TournamentDraftValues }
  | { success: false; state: CreateTournamentState };

function parseTournamentFormData(formData: FormData): ParseResult {
  const raw = extractRawTournamentInput(formData);
...
function sanitizeTournamentUrls(data: TournamentDraftValues) {
...
// 新規タグ名からTagレコードをupsertし、既存タグIDと合わせた最終的なタグID一覧を返す。
async function resolveTagIds(data: TournamentDraftValues): Promise<string[]> {
  ...
  return [...new Set([...data.tagIds, ...createdTagIds])];
}

// create/updateで共通するPrismaのdataフィールド(スラッグ・主催者・タグ以外の全項目)
function buildTournamentDataFields(
  data: TournamentDraftValues,
  sanitizedUrls: Record<string, string | null>
) {
  return {
    name: data.name,
...
export async function createTournamentDraft(
  ...
  const sanitizedUrls = sanitizeTournamentUrls(data);
  const allTagIds = await resolveTagIds(data);
  ...
  const tournament = await prisma.tournament.create({
    data: {
      ...
      organizerId: user.id,
      publishStatus: "DRAFT",
      ...buildTournamentDataFields(data, sanitizedUrls),
      tags: {
